...
// crypto: 16バイトのデータをAESで1ブロック暗号化
void AES128_ECB_indp_crypto(uint8_t *input, uint8_t *output) {
...
int main() {
  ...
  // [oracle:plain text]
  uint8_t plain[16] = {0xcb, 0xa9, 0xdc, 0x00, 0x3f, 0xc8, 0xfa, 0xdd,
                       0xb8, 0xc8, 0x0e, 0x30, 0x88, 0xd3, 0x56, 0x51};

  // [oracle:cipher text] (バイト列形式)
  uint8_t expected[16] = {0x52, 0x6d, 0xe1, 0xdd, 0xcc, 0xab, 0xac, 0xb7,
  ...
  AES128_ECB_indp_crypto(plain, output);

  // 結果の表示
  print_block("Key    ", key);
  print_block("Plain  ", plain);
  print_block("Output ", output);
  print_block("Expect ", expected);
